...
if [ "$EXO_PROFILES" = "" ] ; then 
	EXO_PROFILES="-Dexo.profiles=default,workflow"
fi
...
JMX_OPTS="-Dcom.sun.management.jmxremote -Dcom.sun.management.jmxremote.port=9999 -Dcom.sun.management.jmxremote.ssl=false -Dcom.sun.management.jmxremote.authenticate=false -Djava.rmi.server.hostname=10.11.1.114"
JAVA_OPTS="-Xms4g -Xmx4g -XX:MaxPermSize=512m $JAVA_OPTS $LOG_OPTS $SECURITY_OPTS $EXO_OPTS $EXO_PROFILES $EXO_XML $JMX_OPTS"
export JAVA_OPTS
